...
class Player():
    class Node():
        def __init__(self, inittheta=0, inittime=0, initcost=-999999,inttotal=99999):
    ...
    def is_strategy_available(self, targetcell, MyNode, allcells):
        ...
        for t in range(total_time + add_time):
            ...
            if player.dead:  # 玩家自己死亡，返回false
                return 0
        delta_s = float(player.radius**2 - init_radius**2)
        delta_s_pertime = float(math.log10(delta_s)/((total_time)**2))
        print("000%f"%(delta_s_pertime))
        print("totaltime"+"0%f"%(total_time))
        print("deltajing"+"0%f"%(delta_s))
