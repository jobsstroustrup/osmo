...
class Player():
    ...
    def strategy(self, allcells):
        ...
        if self.finish:
            strategylst = []
            for cell in allcells:
                if cell.distance_from(allcells[self.id]) < r and cell != allcells[
                    self.id] and not cell.dead and cell.radius < allcells[self.id].radius:
            ...
            for starte in strategylst:
                a = self.is_strategy_available(starte[0], starte[1], allcells)
                print(a)
                if a >= 2:
