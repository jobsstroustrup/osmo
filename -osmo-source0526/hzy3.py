...
"""
Osmo Game AI: Layer 1 search and decision mechanism

Author: Zhenyu He (GitHub: jobsstroustrup)
Course: Peking University, Data Structures and Algorithms (Python), SESSDSA 2019, Spring semester
Team: Zhenyu He (primary AI / Layer 1), teammates szx and others (Layers 2-5, engine, opponents)

LAYER 1 DESIGN (hzy3.py, this file, 577 lines):

Core Innovations:
1. Relative Coordinate System + Angular Search
   Observation: optimal ejection angle lies between relative-position angle and relative-velocity angle.
   Search space reduced from 2D (full action space) to 1D (angular interval) via physics intuition.
   Result: tractable enumeration at ~1 degree resolution.

2. Torus Periodic Boundary (9-Region Expansion)
   Game world wraps at boundaries (toroidal topology). Target may be closest in real position
   or one of 4 mirror positions. Generate 4 relevant positions per target, compute all, return best.

3. Forward Simulation Verification (200-Frame Rollout)
   is_strategy_available does not estimate viability heuristically. Instead, it runs actual
   200 frames of game simulation to verify:
     (a) not eaten en route, (b) final state not endangered, (c) target reached, (d) side effects tracked
   Requires complete reverse-engineering of kernel.py physics (see myeject/myabsorb/myupdate).

4. Custom Data Structure: Doubly-Linked Node
   Node stores (theta, time, cost, totaltime) for candidate decisions. Used to build strategy
   trees (unused in final implementation but indicates design thinking).

Key Methods:
  getTheBestNode(sel, oth): expand torus, pick best mirror position
  relative_exchange(sel, oth): compute relative coords, enumerate angles, cost each
  is_strategy_available(plan): run 200-frame sim to verify plan viability
  dangerouscelllst: identify threatening larger cells
  cost_of_theta: estimate ejection mass cost for a given angle
  Plus 15+ utility functions for geometry, bounds checking, angle normalization.

Physics Reverse-Engineering (in support functions):
  myeject: replicate kernel.Cell.eject() (momentum conservation, mass ratio)
  myabsorb: replicate kernel.absorb() (multi-body collision, largest survives)
  myupdate: replicate kernel.Cell.move() (position + velocity update per frame)

ARCHITECTURE CONTEXT (Layers 2-5 in other files):
  Layer 2 (mechanic.py): same search skeleton, alternative parameter tuning
  Layer 3 (strategy.py): strategy orchestration (reactive hazard avoidance + deliberative goal planning)
  Layer 4 (starter provided): game engine (world.py, kernel.py, cell.py, gui.py); Zhenyu reverse-engineered
  Layer 5 (sample/ directory): opponent AI library (brownian motion baseline through sophisticated opponents)

Tournament context: this AI configuration was part of the team's submission that placed 2nd
in the SESSDSA 2019 Spring course tournament.

See README.md and CONTRIBUTORS.md for full architecture and team context.
"""
